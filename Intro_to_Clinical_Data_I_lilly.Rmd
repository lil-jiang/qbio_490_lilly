...
```{r}
write.csv(clinic, "/Users/lilly/Desktop/QBIO490/qbio_490_lilly/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/Users/lilly/Desktop/QBIO490/qbio_490_lilly/analysis_data/brca_clinical_data.csv")
```
...
str(clinic)
head(clinic)
# 1. There are 1174 rows and 114 columns. 
# 2. The data type of $gender is character.
# 3. The age of the 5th patient is 50 years.
# 4. The age of the 1st patient at initial pathological diagnosis is 55 years. The number of days to the last followup from the initial diagnosis is 4047 (around 11 years). This means the patient was around 66 years old at their last followup.
```
...
```{r Exercise 4.1}
# create plot here
lymph_count <- ifelse(clinic$lymph_node_examined_count > 30, "green",
       ifelse(clinic$lymph_node_examined_count > 10, "purple", "pink") )

plot(x = clinic$days_to_last_followup, y = clinic$lymph_node_examined_count,
     main = "Days to Last Followup vs. Number of Lymph Nodes Examined",
     xlab = "Days to Last Followup",
     ylab = "Number of Lymph Nodes Examined",
     col = lymph_count)
...
boxplot(formula = clinic$age_at_initial_pathologic_diagnosis ~ clinic$race_list,
        ...
        main = "Age at Initial Diagnosis vs. Race")
```
...
```{r}
under35_over60_mask <- ifelse(clinic$age_at_initial_pathologic_diagnosis < 35 | clinic$age_at_initial_pathologic_diagnosis > 60, TRUE, FALSE)
```
