...
write.csv(clinical, "/Users/lilly/Desktop/QBIO490/qbio_490_lilly/analysis_data/brca_clinical_data.csv", row.names = FALSE)
```
...
The @data df has many duplicated barcodes in the Tumor_Sample_Barcode column, while the @clinical.data df has different barcodes in each row. The data in @clinical.data df also are in quotes (string type).
...
Since the p-value is less than a significance level of 0.05, we can conclude that vital status and breast carcinoma estrogen receptor status are dependent on each other.

Does this make sense given the biology of cancer?
Yes, because the existence of the hormone receptor directly influences cancer growth and treatment options, which are closely correlated with vital status. 
...
The number of patients that have both genes
...
Since the p-value is significantly less than a significance level of 0.05, we can conclude that the mutations are co-occurent. 
...
ggsave("Users/lilly/Desktop/QBIO490/qbio_490_lilly/week7_maf/oncoplot.png")
```
...
ggsave("Users/lilly/Desktop/QBIO490/qbio_490_lilly/week7_maf/coOncoplot.png")
```
...
Hypothesized Reason: Since TTN plays a big role in muscle contraction and movement, it makes sense that there would be more older patients who have this mutation, as with old age often comes the deterioration of free movement in skeletal muscle. Thus, the mutation rate of this gene probably increases with age. 
...
ggsave("Users/lilly/Desktop/QBIO490/qbio_490_lilly/week7_maf/lollipopPlot.png")
```
...
Yes because specific regions of the protein might be associated with some body function that is affected as a whole.

- What kinds of mutations are the most common?
Missense mutations

- Are there any other interesting observations?
There are some frameshift mutations that seem to only be apparent in old patients. 
...
Patients with mutated TP53 genes seem to be more likely to survive. 

What can you conclude from your plot?
The survival probability for mutated TP53 genes and wild-type TP53 genes aren't significantly different, however it is slightly higher for mutated TP53 genes at around days 2000-4000. 

Is this what you would expect according to the literature? Why or why not?
No, because mutations in the TP53 increase the probability of getting cancer since the function of the gene itself is to suppress tumor cells and prevent cancer formation. Therefore, it would be expected for TP53 mutations to decrease survival probability. 
